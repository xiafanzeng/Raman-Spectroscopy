    ...
    if not os.path.exists('../raman_spectra/'+cas_num+'_NIST_IR.jdx'):
        ...
        if data.text == '##TITLE=Spectrum not found.\n##END=\n':
            ...
            if data.status_code == 200:
                open('../raman_spectra/'+cas_num +
                     '_NIST_IR.jdx', 'wb').write(data.content)
...
def add_jdx(filename, label=None):
    ...
    if label is None:
        ...
        print(
            '{} loaded into the dictionary - shoyu_data_dict.p'.format(data['title']))
    else:
        shoyu_data_dict.update({label: data})
        print('{} loaded into the dictionary - shoyu_data_dict.p'.format(label))
    pickle.dump(shoyu_data_dict, open(
        '../raman_spectra/shoyu_data_dict.p', 'wb'))
...
def more_please(cas_num, label=None):
    ...
    cas_num = ''.join(cas_num.split('-'))
    download_cas(cas_num)
    shoyu_data_dict = add_jdx(
        '../raman_spectra/'+cas_num+'_NIST_IR.jdx', label)
    return shoyu_data_dict
...
def combine_spectra(compound_1, compound_2, plot=False):
    ...
    if plot:
        ...
        plt.plot([i[0] for i in data1], [i[1]
                                         for i in data1], 'b--', label=compound_1['title'])
        plt.plot([i[0] for i in data2], [i[1]
                                         for i in data2], 'g--', label=compound_2['title'])
        plt.plot(x_combined, y_combined, 'r',
                 label='Combination', linewidth=2, alpha=0.7)
        plt.legend()
